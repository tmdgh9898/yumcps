...
async function processUploadedFile(db, file) {
  ...
  try {
    ...
    await db.transaction(async (txDb) => {
      await txDb.run('DELETE FROM daily_logs WHERE date = ?', [dateStr]);
      await txDb.run('DELETE FROM professor_stats WHERE date = ?', [dateStr]);
      await txDb.run('DELETE FROM professor_cases WHERE date = ?', [dateStr]);
      await txDb.run('DELETE FROM professor_case_classifications WHERE date = ?', [dateStr]);
      await txDb.run('DELETE FROM professor_case_checks WHERE date = ?', [dateStr]);
